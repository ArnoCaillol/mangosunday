...
- JS creates class names that exist only in CSS, never in the HTML: `.date`, `.jour`, `.salle`, `.lieu`, `.billets`, `.complet`, `.vu`, `.lien-icone`, `.reseaux-membre`, `.bio-membre`, and the `data-anim` attribute. (`.tri` is the exception — it is in `index.html` *and* recreated by `facades.js`.)
- **A co-applied modifier must sit AFTER its base class in `site.css`.** The house style is co-application, not BEM, so `.reseaux-membre a` and `.reseaux a` collide at equal specificity and only source order decides. `.reseaux-membre` first lived in zone ④ while `.reseaux` lives in zone ⑦, which silently killed the compact `min-height` — the rules now sit directly after `.reseaux`. Zone ordering and cascade ordering pull in opposite directions here; the cascade wins.
...
- **Platform keys have two consumers in facades.js**, `etiquette()` for the label and `ICONES`/`iconePlateforme()` for the mark — `spotify, bandcamp, youtube, instagram, deezer, apple` for the band, plus `tiktok, site` for members. Both are additive: all three renderings iterate the JSON's own keys, never the maps'. A key missing from `etiquette()` falls back to `|| cle` (raw lowercase); a key missing from `ICONES` makes `lienPlateforme` **drop the `lien-icone` class and print the label as text instead**, so a new platform degrades to a readable pill rather than a mute one. That fallback is the reason the class is applied from JS rather than by a blanket selector.
- **The icons are hand-drawn inline SVG, and they have to be.** `img-src 'self' data:` plus the no-third-party rule rule out an icon CDN and an icon font; an external `<use href="sprite.svg#x">` is out too, because external `<use>` content does not inherit the document's `currentColor`, which is what lets one mark serve the cream pill and its mangue hover. They are built with `createElementNS` — `innerHTML` stays banned even for markup we wrote ourselves. Stroke width, caps and joins live in `site.css` (`.lien-icone svg`), not in the shape data, so the whole set restyles from one rule. The `deezer` mark is an acknowledged approximation, flagged as such in its comment.
- **`.membres > li` needs the child combinator, and so does any new rule aiming at the card.** A member card contains a nested `<ul>` of social links, so the descendant form `.membres li` also matched *those* `<li>` — every social link inherited the full card treatment (2px border, 8px radius, `--blanc-casse` ground, offset shadow, 12px padding). Instagram and Apple Music are themselves drawn as rounded squares, so the result was card ▸ pill ▸ logo: three nested rounded rectangles around one icon. Nothing errored, it just looked wrong. `.galerie li` remains a descendant selector but only sets `margin`, and the gallery holds no nested list.
- **Member social links deliberately carry no pill chrome** — `border`, `background` and `box-shadow` are all cleared, while the band's two rows keep theirs. The card is already the container, so a bordered pill inside it re-stated the box. Same icons, two presentations, on purpose: the band's rows read as a set of badges and have no parent box.
